...
export function Sidebar({ 
...
}: SidebarProps & { side?: 'left' | 'right', showFullscreen?: boolean }) {
  ...
  if (!isExpanded && isMobile) {
    return (
      <div 
        className={`sidebar-container fixed ${isLeft ? 'left-0' : 'right-0'} top-0 h-full w-[7rem] bg-slate-900/90 backdrop-blur-md ${isLeft ? 'border-r' : 'border-l'} border-white/10 flex flex-col items-center py-2 gap-4 shadow-2xl z-50 transition-all`}
      >
        {!isLeft && diceContent && (
  ...
  return (
    <div className={`sidebar-container ${isMobile ? `fixed ${isLeft ? 'left-0' : 'right-0'} top-0 w-52 shadow-[0_0_50px_rgba(0,0,0,0.5)]` : `relative w-80`} h-full bg-slate-900 ${isLeft ? 'border-r' : 'border-l'} border-white/10 flex flex-col p-2 md:p-6 gap-2 md:gap-8 z-50 overflow-y-auto transition-all`}>
      ...
      {!isLeft && <div className="flex-grow" />}
